...
provides a numeric lock screen, a minimal app launcher, an in-process System
app, one-finger app-closing gestures, physical power/volume key policy, and a
root-only Unix-socket control interface for development over ADB.
...
```text
src/a26-shell/          Rust crate and committed artwork
...
`aarch64-unknown-linux-musl` target, `aarch64-linux-musl-gcc`, Python 3 with
Pillow, and ADB.
...
- System is still an in-process scene rather than an external app process.
